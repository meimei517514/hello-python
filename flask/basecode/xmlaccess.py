### -*- coding: UTF-8 -*-:
import os,os.path,re,time,HTMLParser
    ...
    with open("basecode/path.txt") as path:
        ...
        for line in path.readlines():
            ...
            if not line.startswith("#"): 
                ...
                line_info=map((lambda x:x.strip("\n")),line_info)
...
def get_sheetdata(sheet_data):
    ...
    row_data=map((lambda row:"~".join(row).rstrip("~ ").split("~")),row_data)
...
def get_celldata(row_data):
    ...
    if len(row_data)<1000:
        ...
        for cell in row_data:
            ...
            data=data.decode("utf-8") if isinstance(data,str) else data

            data=html_parser.unescape(data)

            cell_data.append(data)
    '''
    if "210108" in cell_data:
