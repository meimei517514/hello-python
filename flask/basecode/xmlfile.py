...
def get_usefulbranch(branch):
    ...
    if last_slice>11:
        ...
        return True if gap_days<=25 else False
    else:
        return False
...
def get_filediff(selected_hash,selected_sheet,diff_detail,c_type,path_name):
    ...
    if ".xml" in path_name and "M" in c_type:
        ...
        other_diff=[u"都是些无用的变更，表格的数据没变，莫慌"] if not sheet_name else []
...
def extract_rowdata(selected_sheet,row_refer,row_fulldata,row,sheet_area):
    ...
    #print row_refer,reference_row,len(row_fulldata)
...
def map_truediff(row_cinfo,sheet_header):
    ...
    diff_data=[[u"都是些无用的变更，表格的数据没变，莫慌"]] if not diff_data else diff_data
...
def get_validrow(diff_list,style):
    ...
    filtered_diff=filter((lambda x:not x.startswith(ctag[style])),diff_list)
...
def get_sheetrange(path_name):
    ...
    sheet_area=zip(sheet_start,sheet_end)

    sheet_range=dict(zip(sheet_name,sheet_area))

    sheet_list=[ (start,xmlfile_list[start:end]) for start,end in sheet_area]

    sheet_list=dict(zip(sheet_name,sheet_list))

    return sheet_range,sheet_list
...
def get_rowrange(start,sheet_info):
    ...
    #获得每行的行范围
    row_start=[ start+key  for key,line in enumerate(sheet_info) if "<Row" in line ]

    row_end=[ start+key  for key,line in enumerate(sheet_info) if "/Row>" in line  or ( "<Row" in line and "/>" in line)]
